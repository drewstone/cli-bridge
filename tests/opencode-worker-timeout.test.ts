...
/** The PATH wrapper must use the same deadline as the bridge request. */
describe('opencode worker timeout propagation', () => {
      ...
      const stream = backend.chat(
        {
          ...
          ...(requestTimeoutMs === undefined
            ? {}
            : { execution: { kind: 'host', timeoutMs: requestTimeoutMs } }),
  ...
  it('lets the request override both backend fallback and ambient wrapper config', async () => {
    const env = await spawnEnvFor(
      300_000,
      14_400_000,
      { OPENCODE_RUN_TIMEOUT_SECONDS: '600' },
    )
    expect(env.OPENCODE_RUN_TIMEOUT_SECONDS).toBe('14400')
  })
