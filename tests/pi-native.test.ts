...
describe('Pi native RPC adapter', () => {
  ...
  it('uses a real JSONL child for two HTTP turns, state proof, and canonical replay', async () => {
    ...
    const backend = new PiBackend({ bin: 'pi', timeoutMs: 10_000, spawner: makeChildSpawner(calls, lifecycle) })
  ...
  it('releases the native child lease when the RPC process exits unexpectedly', async () => {
    ...
    const created = await app.request('/v1/sessions', {
      method: 'POST',
      body: JSON.stringify({ id: 'crash-session', model: 'pi/test', cwd: dir }),
    })
    expect(created.status).toBe(201)
  ...
  it('retains the native lease and private files until termination is proven', async () => {
    ...
    const spawner: Spawner = async (_bin, args, options) => {
    ...
    const backend = new PiBackend({ bin: 'pi', timeoutMs: 1_000, spawner })
  ...
  it('aborts and closes a real retained child exactly once on cancellation', async () => {
    ...
    expect((await app.request('/v1/sessions', {
      method: 'POST',
      body: JSON.stringify({ id: 'cancel-real', model: 'pi/test', cwd: dir }),
    })).status).toBe(201)
  ...
  it('bounds an unacknowledged initial prompt RPC and then closes the real child', async () => {
    ...
    expect((await app.request('/v1/sessions', {
      method: 'POST',
      body: JSON.stringify({ id: 'timeout-real', model: 'pi/test', cwd: dir }),
    })).status).toBe(201)
  ...
  it('acknowledges a Pi interaction only after the native turn visibly resumes', async () => {
    ...
    expect((await app.request('/v1/sessions', {
      method: 'POST',
      body: JSON.stringify({ id: 'native-interaction', model: 'pi/test', cwd: dir }),
    })).status).toBe(201)
    ...
    const command = {
      operationId: 'native-interaction-response',
      binding: {
        runId: 'native-interaction-run',
        environmentId: 'cli-bridge',
        sessionId: 'native-interaction',
  ...
  it('returns transport failure when Pi consumes a response but exposes no resumed native effect', async () => {
    ...
    await app.request('/v1/sessions', {
      method: 'POST',
      body: JSON.stringify({ id: 'silent-interaction', model: 'pi/test', cwd: dir }),
    })
    ...
    const response = await app.request(
      `/v1/runs/silent-interaction-run/interactions/${event.request.id}/respond`,
      {
        method: 'POST',
        body: JSON.stringify({
          operationId: 'silent-interaction-response',
          binding: {
            runId: 'silent-interaction-run',
            environmentId: 'cli-bridge',
            sessionId: 'silent-interaction',
            interactionId: event.request.id,
          },
          response: {
            id: event.request.id,
