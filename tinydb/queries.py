...
>>> ((where('f1') == 5) & (where('f2') != 2)) | where('s').matches(r'^\\w+$')  # doctest: +ELLIPSIS
QueryImpl('or', frozenset({...}))
...
class Query(QueryInstance):
    ...
    >>> User = Query()
    >>> db.search(User.name == 'John Doe')  # doctest: +SKIP
    >>> db.search(User['logged-in'] == True)  # doctest: +SKIP

    2) Classical usage:

    >>> db.search(where('value') == True)  # doctest: +SKIP
    ...
    >>> # Binary AND:
    >>> db.search((where('field1').exists()) & (where('field2') == 5))  # doctest: +SKIP
    >>> # Binary OR:
    >>> db.search((where('field1').exists()) | (where('field2') == 5))  # doctest: +SKIP
    ...
    def test(self, func: Callable[[Mapping], bool], *args) -> QueryInstance:
        ...
        ...     return val == 42
        ...
        >>> Query().f1.test(test_func)  # doctest: +ELLIPSIS
        QueryImpl('test', ('f1',), <function test_func at 0x...>, ())
    ...
    def any(self, cond: Union[QueryInstance, List[Any]]) -> QueryInstance:
        ...
        QueryImpl('any', ('f1',), QueryImpl('==', ('f2',), 1))
    ...
    def all(self, cond: Union['QueryInstance', List[Any]]) -> QueryInstance:
        ...
        QueryImpl('all', ('f1',), QueryImpl('==', ('f2',), 1))
    ...
    def one_of(self, items: List[Any]) -> QueryInstance:
        ...
        QueryImpl('one_of', ('f1',), ('value 1', 'value 2'))
